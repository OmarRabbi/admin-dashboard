...
function Navbar({ handleMenuIcon, handleShowNavbar, showProfileNavbar }) {
  return (
            ...
            <div className="relative rounded-lg border bg-slate-900 text-gray-50 border-opacity-20 py-2.5 px-2 sm:px-4 flex items-center gap-1.5 sm:gap-3">
              ...
              {showProfileNavbar && (
                <div
                  ...
                  className="z-[20] w-full absolute left-0 top-[110%] right-0 flex flex-col gap-2 sm:gap-6 py-2.5 px-2 sm:p-4 text-light bg-[#050E13] border-[0.75px] border-white border-opacity-10 rounded-md h-max"
                >
                  {menuItems.map(({ id, label, icon, path }) => (
                    ...
                    >
                      <img
                        src={icon}
                        alt="icon"
                        className="w-3 sm:w-4 h-3 sm:h-4 shrink-0"
                      />
                      <p
                        className={`text-sm sm:text-base font-semibold shrink-0 leading-6 ${
                          activeItem === id ? "text-white" : "text-[#B5BCC1]"
                        }`}
                      >
                        {label}
                      </p>
